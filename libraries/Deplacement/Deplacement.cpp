...
/****** Test Asserv ******/
  ...
  if((type!=EVITEMENT && type!=EFFECTEUR) && ((erreur_position>-precision && erreur_position<precision) 
    && (erreur_position2>-precision && erreur_position2<precision))//){ 
    && consigne==arrayConsigne[currentConsigne]){ //A REMETTRE ABSOLUMENT AVEC TABLEAU ARRAY_CONSIGNE !!!!!!!!!!!!
    currentConsigne++;
